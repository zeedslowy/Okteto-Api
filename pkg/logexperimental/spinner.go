...
func newSpinner(w OktetoWriter) *sp.Spinner {
	...
	spinner.PreUpdate = func(spinner *sp.Spinner) {
		...
		if err != nil {
			w.Infof("failed to get terminal size: %s", err)
		}
...
func (sl *spinnerLogger) Spinner(text string) {
	sl.sp.Suffix = fmt.Sprintf(" %s", ucFirst(text))
	sl.sp.FinalMSG = sl.sp.Suffix
}

// StartSpinner starts to run the spinner if enabled or Println if not
func (sl *spinnerLogger) StartSpinner() {
	if sl.spinnerSupport {
		if sl.sp.FinalMSG == "" {
			sl.sp.FinalMSG = sl.sp.Suffix
		}
		sl.sp.Start()
	} else {
		sl.writer.Println(strings.TrimSpace(sl.sp.Suffix))
	}
}

// StopSpinner deletes FinalMSG and stops the running of the spinner
func (sl *spinnerLogger) StopSpinner() {
	if sl.sp.FinalMSG != "" {
		sl.sp.FinalMSG = ""
	}
	if sl.spinnerSupport {
