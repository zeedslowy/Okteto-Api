...
func TestBuildCommandV1(t *testing.T) {
	...
	expectedImage := fmt.Sprintf("%s/%s/test:okteto", oktetoContext.Context().Registry, testNamespace)
...
func TestBuildCommandV2(t *testing.T) {
	...
	expectedAppImage := fmt.Sprintf("%s/%s/%s-app:okteto", oktetoContext.Context().Registry, testNamespace, filepath.Base(dir))
	require.False(t, isImageBuilt(expectedAppImage))

	expectedApiImage := fmt.Sprintf("%s/%s/%s-api:okteto", oktetoContext.Context().Registry, testNamespace, filepath.Base(dir))
...
func TestBuildCommandV2OnlyOneService(t *testing.T) {
	...
	expectedImage := fmt.Sprintf("%s/%s/%s-app:okteto", oktetoContext.Context().Registry, testNamespace, filepath.Base(dir))
	require.False(t, isImageBuilt(expectedImage))
...
func TestBuildCommandV2VolumeMounts(t *testing.T) {
	...
	expectedBuildImage := fmt.Sprintf("%s/%s/%s-vols:okteto", oktetoContext.Context().Registry, testNamespace, filepath.Base(dir))
	require.False(t, isImageBuilt(expectedBuildImage))

	expectedImageWithVolumes := fmt.Sprintf("%s/%s/%s-vols:okteto-with-volume-mounts", oktetoContext.Context().Registry, testNamespace, filepath.Base(dir))
...
func TestBuildCommandV2Secrets(t *testing.T) {
	...
	expectedBuildImage := fmt.Sprintf("%s/%s/%s-test:okteto", oktetoContext.Context().Registry, testNamespace, filepath.Base(dir))
	require.False(t, isImageBuilt(expectedBuildImage))
