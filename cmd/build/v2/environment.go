...
func (bc *OktetoBuilder) SetServiceEnvVars(service, reference string) {
	...
	if err != nil {
		bc.logger.Debugf("could not set service env vars: %w", err)
		return
	}

	bc.logger.Debugf("envs registry=%s repository=%s image=%s tag=%s", ref.Registry, ref.Repo, ref.Image, ref.Tag)
	...
	bc.logger.Debug("manifest env vars set")
}
